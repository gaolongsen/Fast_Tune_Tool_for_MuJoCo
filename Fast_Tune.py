...
ref_body_name = "satellite"
xml_test_name = "/home/wam/sat_demo.xml"
...
class Controller():
    ...
    def _rot(self, axis: int, direction: Direction):
        ...
        if axis == 2:
            ...
            self.sim.data.set_mocap_quat("endeff_des",
                                         np.array([r.as_quat()[3], r.as_quat()[0], r.as_quat()[1], r.as_quat()[2]]))
            self.sim.step()
        elif axis == 1:
    ...
    def _move(self, axis: int, direction: Direction):
        ...
        """
        if axis == 2:
            self.sim.data.set_mocap_pos("endeff_des",
                                        self.sim.data.mocap_pos + np.array([0, 0, self.pos_speed * direction.value]))
            self.sim.step()
        elif axis == 1:
            self.sim.data.set_mocap_pos("endeff_des",
                                        self.sim.data.mocap_pos + np.array([0, self.pos_speed * direction.value, 0]))
            self.sim.step()
        elif axis == 0:
            self.sim.data.set_mocap_pos("endeff_des",
                                        self.sim.data.mocap_pos + np.array([self.pos_speed * direction.value, 0, 0]))
            self.sim.step()
        else:
            pass
...
class Viewer(MjViewer):
    ...
    def key_callback(self, window, key, scancode, action, mods):
        ...
        elif key == glfw.KEY_F4: # Show the diff between the current ref frame with body frame
            self.rel_pos = True

        elif key == glfw.KEY_F3: # Close the diff between the current ref frame with body frame
            self.rel_pos = False
        else:
            super().key_callback(window, key, scancode, action, mods)

    def add_extra_menu(self, pos, rot, bpos, brot):
        self.add_overlay(
            const.GRID_TOPRIGHT,
        ...
        self.add_overlay(const.GRID_TOPRIGHT, "W-Position", str(np.array(['%.2f' % elem for elem in pos])))
        self.add_overlay(const.GRID_TOPRIGHT, "W-Rotation", str(np.array(['%.2f' % elem for elem in quat2euler(rot)])))
        self.add_overlay(const.GRID_TOPRIGHT, " "," ")
        self.add_overlay(const.GRID_TOPRIGHT, "B-Position", str(np.array(['%.2f' % elem for elem in bpos])))
        self.add_overlay(const.GRID_TOPRIGHT, "B-Rotation", str(np.array(['%.2f' % elem for elem in brot])))


def main():
    # load model
    model = load_model_from_path(xml_test_name)
    sim = MjSim(model)
    # viewer set up
    viewer = Viewer(sim)
    l1_id = sim.model.body_name2id(ref_body_name)
    lookat = sim.data.body_xpos[l1_id]
    ...
    # postion offset
    sim.data.set_mocap_pos("endeff_des", np.array([0, 0, 0]))
    bpos_ref = sim.data.get_body_xpos(ref_body_name)
    brot_ref = sim.data.get_body_xquat(ref_body_name)

    while True:
        pos = sim.data.get_mocap_pos("endeff_des")
        rot = sim.data.get_mocap_quat("endeff_des")

        bpos = sim.data.get_mocap_pos("endeff_des") - bpos_ref
        brot = quat2euler(sim.data.get_mocap_quat("endeff_des")) - quat2euler(brot_ref)

        sim.step()
        viewer.render()
        viewer.add_extra_menu(pos, rot, bpos, brot)
